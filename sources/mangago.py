...
base_url = "https://www.fashionlib.net"
...
def get_url(series: str, range_: Iterable, project_path: str):
    ...
    """
    first_url = f"http://www.mangago.me/read-manga/{series}{get_url_structure(series)}"
    ...
    for chapter, info in tqdm(chapters.items(), desc="Getting chapter page counts", unit="chapters"):
        ...
        chapters[chapter]["page_count"] = \
            int(pages_element.find_all("a")[-1].text.split("Pg ")[-1])

    # Get image urls
    for chapter, info in tqdm(chapters.items(), desc="Getting chapter image urls", unit="chapters"):
        for page in range(1, info["page_count"]+1):  # +1 so 5eps, range(1, 6)
            full_url = base_url + info["sub_url"] + str(page)
            r = session.get(full_url)
            r.html.render()
            soup = BeautifulSoup(
                r.html.html, features=html_parser)
            if soup.find("img", class_=f"page{str(page)}") == None:
                # <canvas> pages (temporarily skip for now)
                continue
...
def get_length(series: str) -> int:
    ...
    """
    url = f"http://www.mangago.me/read-manga/{series}{get_url_structure(series)}"
...
@lru_cache
def get_url_structure(series: str) -> str:
