    ...
    if output_format == "combined":  # TODO: other methods
        ...
        for chapter, info in chapters.items():
            ...
            for page in range(len(info[urls_key])):
                Image.open(os.path.join(project_path, chapter, f"{page+1}.{file_format}"))\
                    .convert('RGB')\
                    .save(os.path.join(project_path, chapter, f"{page+1}.jpg"))
                image_list.append(
                    os.path.join(project_path, chapter, f"{page+1}.jpg"))
...
# output("webtoon 329660 1-124", "./downloads\webtoon 329660 1-124",
#        "./downloads", "combined", "jpg")
